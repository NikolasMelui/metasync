...
metatests.test('successful some with array', test => {
  const arr = [1, 2, 3];
...
metatests.test('some with empty', test => {
  const arr = [];

  metasync.some(arr, test.mustNotCall(), (err, accepted) => {
    test.error(err);
...
metatests.test('some with error', test => {
  const arr = [1, 2, 3];
