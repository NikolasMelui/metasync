...
metatests.test('reduceRight with initial and array', test => {
  const arr = [1, 2, 3, 4, 5];
  const initial = 10;
...
metatests.test('reduceRight empty items with initial', test => {
  const arr = [];
  const initial = 10;
...
metatests.test('reduceRight empty items without initial', test => {
  const arr = [];
  const expectedError = new TypeError(
    'Reduce of consumed async iterator with no initial value'
  );

  metasync.reduceRight(arr, test.mustNotCall(), (err, res) => {
...
metatests.test('reduceRight single-element array without initial', test => {
  const arr = [2];
...
metatests.test('reduceRight with error', test => {
  const str = '10120011';
  const reduceRightError = new Error('reduceRight error');
  ...
  metasync.reduceRight(
    ...
    (prev, cur, callback) =>
      process.nextTick(() => {
        const digit = +cur;
        if (digit > 1) {
          callback(reduceRightError);
          return;
        }
        callback(null, prev * 2 + digit);
      }),
    (err, res) => {
      test.isError(err, reduceRightError);
      test.strictSame(res, undefined);
      test.end();
    }
  );
