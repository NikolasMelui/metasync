...
function ArrayChain(array, ready = true) {
  this.array = array;
  this.chain = [];
  this.ready = ready;
}
...
ArrayChain.prototype.fetch = function(fn) {
  const newChain = new ArrayChain(this.array, false);
  this.chain.push({
    op: 'fetch',
    fn: (error, result, next) => {
      fn(error, result, (err, data) => {
        if (err) {
          next(err);
        } else {
          next(null, data);
          newChain.array = data || this.array;
          newChain.ready = true;
          newChain.execute();
        }
      });
